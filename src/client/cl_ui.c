...
void LAN_LoadCachedServers() {
	int size;
	fileHandle_t fileIn;
	cls.numglobalservers = cls.numfavoriteservers = 0; // L0 - ipv6
	cls.numGlobalServerAddresses = 0;
	if (FS_SV_FOpenFileRead("servercache.dat", &fileIn)) {
		FS_Read(&cls.numglobalservers, sizeof(int), fileIn);
		//FS_Read( &cls.nummplayerservers, sizeof( int ), fileIn ); // L0 - ipv6
		FS_Read(&cls.numfavoriteservers, sizeof(int), fileIn);
		FS_Read(&size, sizeof(int), fileIn);
		if (size == sizeof(cls.globalServers) + sizeof(cls.favoriteServers)) { // L0 - ipv6
...
void LAN_SaveServersToCache() {
	...
	size = sizeof(cls.globalServers) + sizeof(cls.favoriteServers); // L0 - ipv6
