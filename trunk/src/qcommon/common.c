...
#endif
			logfile = FS_FOpenFileWrite( "rtcwmpconsole.log" );
...
void Com_ReadCDKey(const char *filename) {
	...
	if (CL_CDKeyValidate(buffer) == AUTH_OK) {
		...
		Q_strncpyz(cl_cdkey, "NULL", 5);
	}

	//Com_Printf("^2Reading KEY");
}
...
void Com_AppendCDKey(const char *filename) {
	...
	FS_SV_FOpenFileRead(fbuffer, &f);
	if (!f) {
		Q_strncpyz(&cl_cdkey[16], "NULL", 5);
		//Com_Printf("^2APPEND - BAILING OUT");
		return;
	}
	...
	if (CL_CDKeyValidate(buffer) == AUTH_OK) {
		...
		Q_strncpyz(&cl_cdkey[16], "NULL", 17);
	}

	//Com_Printf("^2APPENDING KEY");
}
...
void Com_WriteConfigToFile( const char *filename ) {
	...
	FS_Printf( f, "// generated by rtcwMP Client, do not modify\n" );
	Key_WriteBindings( f );
