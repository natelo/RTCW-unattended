...
void SV_MasterHeartbeat( const char *hbname ) {
	static netadr_t	adr[MAX_MASTER_SERVERS][2]; // L0 - ipv6 - [2] for v4 and v6 address for the same address string.
	int i;
	int	res; // L0 - ipv6
	int	netenabled;	// L0 - ipv6

	netenabled = Cvar_VariableIntegerValue("net_enabled"); // L0 - ipv6
	...
	// L0 - ipv6 / rest of the function is a dump from ioquake 
...
qboolean SV_CheckDRDoS(netadr_t from) {
	...
	for (i = 0; i < MAX_INFO_FLOOD_BANS; i++, ban++) {
		if (svs.time - ban->time < 20000 && // Twenty second ban
	...
	if (specificCount >= 6) { // Already sent 6 requests to this IP in last 1.4 second.
		Com_Printf("Possible server flood attempt detected (from address %s). Server is ignoring any requests from this address for the next 20 seconds.\n", NET_AdrToString(exactFrom));
